...
/**
 * The JellyfinImportMetadata class is responsible for importing metadata from a
 * CSV file into a Jellyfin instance.
 * It performs various checks to ensure the integrity of the data and updates
 * the Jellyfin instance accordingly.
 * 
 * @since 1.2.0
 * @version 1.0
...
public class JellyfinImportMetadata {
    ...
    /**
     * Starts the pre-check process for importing metadata from a CSV file into a
     * Jellyfin instance.
     * 
     ...
     * 3. Checks if the file has one or more lines from a different server ID
     * (Instance) or was generated from an older JEMM version.
     * 4. Creates a "From/To" differences log.
     * 
     * @return A JellyfinResponseStandard object containing the result of the
     *         process. If the process is successful, the response code and message
     *         indicate success. If the process fails, the response code and message
     *         indicate the error.
     * @since 1.2.0
     * @version 1.0
     * @author CesarBianchi
        ...
        if (!this.getcOriginPath().isEmpty()) {
            ...
            try {
                ...
                if (processStatus.isSuccess()) {
                    ...
                    // 2nd: Check if any line of file has more or less fields than expected. Needs
                    // to be the same quantity
                    processStatus = this.preCheckLines();
                    if (processStatus.isSuccess()) {

                        // 3rd: Check if the file has one or more lines from different serverID
                        // (Instance) or
                ...
                processStatus.setResponseCode("CSV_IMP_ERROR");
                processStatus.setResponseMessage("An unexpected error occurred during pre-check: " + ex.getMessage());
            }
        } else {
            processStatus.setIsSuccess(false);
     ...
     * 3. Checks if the fields between the file header and the standard header are
     * the same.
     * 
     * @return A JellyfinResponseStandard object containing the result of the check.
     *         If the headers match, the response code and message indicate success.
     *         If the headers do not match, the response code and message indicate
     *         the error.
     * @since 1.2.0
     * @author CesarBianchi
     */
    public JellyfinResponseStandard checkHeader() {
        ...
        if (fileHeader.size() == standardHeader.length) {
            ...
            for (int nI = 0; nI < standardHeader.length; nI++) {
                ...
                if (!field1.equals(field2)) {
                    ...
                    processResult.setResponseMessage(
                            "The fields/columns inside the CSV file are not equal to expected. Please, make sure you are using a CSV Exported File as a sample, with the same fields/columns and structure.");
                    break;
                } else {
                    processResult.setIsSuccess(true);
                    processResult.setResponseCode("CSV_IMP_004");
                    processResult.setResponseMessage(
                            "The fields/columns between the CSV file and the expected are the same. Good job!");
                }
            }
            ...
            processResult.setResponseMessage(
                    "The file had fewer fields/columns than expected. Please, make sure you are using a CSV Exported File as a sample, with the same fields/columns and structure.");
    ...
    /**
     * Checks if each line of the CSV file has the same number of fields as
     * expected.
     * 
     * This method performs the following steps:
     * 1. Sets the default return values.
     * 2. Gets the total number of fields in the standard header.
     * 3. Checks if each line of the CSV file has the same number of fields as the
     * standard header.
     * 
     * @return A JellyfinResponseStandard object containing the result of the check.
     *         If the lines are valid, the response code and message indicate
     *         success. If any line has an incorrect number of fields, the response
     *         code and message indicate the error.
    ...
    public JellyfinResponseStandard preCheckLines() throws FileNotFoundException, IOException {
        ...
        // 3rd: Check if each line of the CSV file has the same number of fields as the
        // standard header
        if (!this.getcOriginPath().isEmpty()) {
            try {
                r = new BufferedReader(new FileReader(this.getcOriginPath()));
                ...
                while (true) {
                    ...
                    if (lineContent == null) {
                        ...
                        if (fieldsinLine.size() != totalFields) {
                            ...
                            cMsg = cMsg.concat("has ").concat(Integer.toString(fieldsinLine.size()))
                                    .concat(" fields/columns ");
    ...
    /**
     * Tokenizes a string into an ArrayList of strings based on a specified
     * delimiter.
     * 
     * @param cContent  The string content to be tokenized.
     * @param delimiter The delimiter to be used for tokenizing the string. If null
     *                  or empty, the default delimiter ";" is used.
     ...
     * 2. For each line, compares the differences between the instance data and the
     * file line.
     * 3. If any field has changed, adds the full line to the array "linesChanged".
     * 
     * @return A JellyfinResponseStandard object containing the result of the
     *         comparison. If the comparison is successful, the response code and
     *         message indicate success. If any line has errors, the response code
     *         and message indicate the error.
     * @since 1.2.0
     * @author CesarBianchi
     */
    private JellyfinResponseStandard compareDiferencesInLine() {
        ...
        // 2nd: For each line, compare the differences between the Instance and the
        // FileLine.
        for (int nI = 0; nI < this.fileLines.size(); nI++) {

            // Deserialize the line into multiple Array items (each position is a field
            // content)
            ...
            if ((instanceItem != null) && (instanceItem.getId() != null) && (instanceItem.getId().equals(itemID))) {
                ...
                cInvalidIDMsg = cInvalidIDMsg
                        .concat(" is invalid. Make sure the selected CSV file is from the same Jellyfin Instance. ");
                this.insertLogComparsionRecord(cInvalidIDMsg);

                processStatus.setIsSuccess(false);
                processStatus.setResponseCode("CSV_IMP_009");
                processStatus.setResponseMessage(
                        "One or more lines has errors. The import process was stopped. Please check log file at "
                                .concat(this.getLogDiferencesFile()));
            }
        }
     ...
     * 
     * This method creates a log entry with the start date, the analyzed CSV file
     * path, and the total number of lines to be processed.
     * 
     * @since 1.2.0
     * @author CesarBianchi
     */
     ...
     * 
     * This method opens the log differences file and writes the provided message to
     * it. If the file does not exist, it creates a new one.
     * 
    ...
    /**
     * Retrieves an item from the Jellyfin instance based on the item ID and parent
     * ID.
     * 
     * @param itemID   The ID of the item to retrieve.
     * @param parentID The ID of the parent item.
     * @return The JellyfinItem object retrieved from the instance, or null if not
     *         found.
     * @since 1.2.0
     * @author CesarBianchi
     */
        ...
        try {
            ...
            for (int nI = 0; nI < insItems.getItems().size(); nI++) {
                ...
                try {
                    ...
                    String logMsg = "Error retrieving item details for item in list (ID: " + item.getId() + "): "
                            + ex.getMessage();
                    Logger.getLogger(JellyfinImportMetadata.class.getName()).log(Level.WARNING, null, ex);
                    this.insertLogComparsionRecord(logMsg);
                }
            }

        } catch (IOException ex) {
            String logMsg = "Error retrieving item ID " + itemID + ": " + ex.getMessage();
            Logger.getLogger(JellyfinImportMetadata.class.getName()).log(Level.SEVERE, null, ex);
            this.insertLogComparsionRecord(logMsg);
        } catch (ParseException ex) {
            String logMsg = "Error parsing item ID " + itemID + ": " + ex.getMessage();
            Logger.getLogger(JellyfinImportMetadata.class.getName()).log(Level.SEVERE, null, ex);
            this.insertLogComparsionRecord(logMsg);
        }
     ...
     * 
     * This method creates a log entry indicating the difference between the old and
     * new values of a field.
     * 
    ...
    /**
     * Checks the origin of the CSV file to ensure it matches the current Jellyfin
     * instance.
     * 
     * This method performs the following steps:
     * 1. Sets the default response.
     * 2. Checks all lines of the file for unrecognized server ID or JEMM version.
     * 3. Verifies if the JEMM version and server ID in the file match the current
     * instance.
     * 
     * @return A JellyfinResponseStandard object containing the result of the check.
     *         If the origin is valid, the response code and message indicate
     *         success. If the origin is invalid, the response code and message
     *         indicate the error.
     * @since 1.2.0
     * @author CesarBianchi
     */
    public JellyfinResponseStandard grantOrigin() {
        ...
        for (int nI = 0; nI < this.fileLines.size(); nI++) {
            ...
            // 1st: Check if JemmVersion present in line is the same as the JemmVersion
            // running
            ...
            // 2nd: Check if ServerID present in line is the same as the ServerID where Jemm
            // is connected
    ...
    /**
     * Starts the import process for importing metadata from a CSV file into a
     * Jellyfin instance.
     * 
     * This method performs the following steps:
     * 1. Transforms each line in the `linesChanged` array into an array of fields.
     * 2. Gets the ID and ParentID, then identifies if the line refers to a Folder
     * or an Item.
     * 3. If the line refers to a Folder, updates the folder in the main object with
     * values changed from the file.
     * 4. If the line refers to an Item, gets the most recent data and updates the
     * main instance object with values changed from the file.
     * 
     * @return A JellyfinResponseStandard object containing the result of the import
     *         process. If the process is successful, the response code and message
     *         indicate success. If any changes were not applied, the response code
     *         and message indicate the error.
     * @since 1.2.0
     * @version 1.0
     * @author CesarBianchi
    ...
    public JellyfinResponseStandard startImport() {
        ...
        try {
            for (int nI = 0; nI < this.linesChanged.size(); nI++) {
                ...
                try {
                    ...
                    // 2nd: Gets the ID, ParentID and then identifies if the line refers to a Folder
                    // or an Item.
                    ...
                    if (isFolder) {
                        if (this.refreshFolder(itemID)) {
                            ...
                            itemSuccess = false;
                            errorReport.append("- Failed to refresh folder ID: ").append(itemID).append("\n");
                        }
                    } else {
                        // Before updating an item, get its most recent data and update the main
                        // instance object
                    ...
                    lUpdated = false;
                    String lineError = "Error processing line " + (nI + 1) + ": " + ex.getMessage();
                    Logger.getLogger(JellyfinImportMetadata.class.getName()).log(Level.SEVERE, lineError, ex);
                    errorReport.append("- ").append(lineError).append("\n");
                }
            }
        } catch (Exception ex) {
            lUpdated = false;
            Logger.getLogger(JellyfinImportMetadata.class.getName()).log(Level.SEVERE, "Critical error in startImport",
                    ex);
            errorReport.append("Critical error stopped import: ").append(ex.getMessage());
        }
        ...
        if (lUpdated) {
            ...
            String detailedMsg = "Import Process done with errors:\n" + errorReport.toString();
            // Truncate if too long for dialog
            if (detailedMsg.length() > 500)
                detailedMsg = detailedMsg.substring(0, 500) + "... \n(See logs for full details)";
            processResult.setResponseMessage(detailedMsg);
        }
    ...
    /**
     * Compares the old and new values of a property and registers the difference if
     * they are not equal.
     * 
     ...
     * @param lineNumber     The line number where the difference was found.
     * @param lChangedBefore Indicates if a change was detected before this
     *                       comparison.
     * @return true if the values are different, false otherwise.
     * @since 1.2.0
     * @author CesarBianchi
     */
    public boolean compareValues(String oldValue, String newValue, String property, String lineNumber,
            boolean lChangedBefore) {
        boolean lChanged = false;
     ...
     * @return true if the folder metadata was successfully refreshed, false
     *         otherwise.
     * @since 1.2.0
     * @author CesarBianchi
     */
